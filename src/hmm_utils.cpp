...
{
  return ((m/2-l) * log(1-rf) + l * log(rf) - log(nChoosek(m/2, l)));
}
...
std::vector<double> transition(int m, std::vector<double>& rf)
{
  ...
  int count = 0;
  int n_rf = rf.size();
  std::vector<double> T(n_rf*g*g*g*g);
  for(int z = 0; z < n_rf; ++z)
  {
    for(int i = 0; i < g; ++i)
    {
      for(int k = 0; k < g; ++k)
      {
        for(int j = 0; j < g; ++j)
        {
          for(int l = 0; l < g; ++l)
          {
            T[count]  = prob_k1_given_k_l_m(m, n_rec_given_genk_and_k1(m, i+1, j+1), rf[z]) *
              prob_k1_given_k_l_m(m, n_rec_given_genk_and_k1(m, k+1, l+1), rf[z]);
            count++;
          }
        }
      }
    ...
    for(int i = 0; i < g; ++i)
    {
      for(int k = 0; k < g; ++k)
      {
        for(int j = 0; j < g; ++j)
        {
          for(int l = 0; l < g; ++l)
          {
            T[count]  = log_prob_k1_given_k_l_m(m, n_rec_given_genk_and_k1(m, i+1, j+1), rf[z]) +
              log_prob_k1_given_k_l_m(m, n_rec_given_genk_and_k1(m, k+1, l+1), rf[z]);
            count++;
          }
        }
      }
...
std::vector<double> rec_num(int m){
  int count = 0, g = nChoosek(m, m/2);
  std::vector<double> R(g*g*g*g);
  for(int i = 0; i < g; ++i)
  {
    for(int k = 0; k < g; ++k)
    {
      for(int j = 0; j < g; ++j)
      {
        for(int l = 0; l < g; ++l)
        {
          R[count] = (n_rec_given_genk_and_k1(m, i+1, j+1) +
                      n_rec_given_genk_and_k1(m, k+1, l+1))/(double)m;
          count++;
        }
      }
    }
